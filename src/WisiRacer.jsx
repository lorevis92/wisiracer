...
const TRACKS = {
  ...
  vortex_gate:   { label:"Vortex Gate",    desc:"Portale energetico — anelli di forza elettrica.",    bg:0x04040f, fog:0x0a0820, fogD:0.0012, amb:0x8866ff, sun:0xaaaaff, nebula:true,  planet:false, rockColor:0x443355, rocks:55, bgImg:"/assets/bg_vortex_gate.jpeg",   pts:[[0,0,0],[200,30,-180],[400,10,-150],[550,50,50],[470,20,280],[220,-40,400],[-50,30,360],[-280,60,420],[-500,10,240],[-600,-30,0],[-460,40,-220],[-180,-20,-300]] },
  crimson_dust:  { label:"Crimson Dust",   desc:"Pianeta desertico — tempeste di polvere cosmica.",   bg:0x120302, fog:0x2a0808, fogD:0.0013, amb:0xff6633, sun:0xff9944, nebula:false, planet:false, rockColor:0x8a3322, rocks:95, bgImg:"/assets/bg_crimson_dust.jpeg",  pts:[[0,0,0],[250,20,-200],[450,0,-130],[580,40,80],[500,-20,320],[250,30,460],[-30,50,420],[-300,10,470],[-520,-20,280],[-610,30,-20],[-470,-10,-250],[-200,40,-330]] },
  dark_matter:   { label:"Dark Matter",    desc:"Il vuoto profondo — filamenti di materia oscura.",   bg:0x010102, fog:0x050510, fogD:0.0008, amb:0x443366, sun:0x8866cc, nebula:false, planet:false, rockColor:0x222233, rocks:30, bgImg:"/assets/bg_dark_matter.jpeg",  pts:[[0,0,0],[300,50,-250],[550,0,-180],[680,60,100],[580,-30,380],[300,40,530],[-60,70,490],[-360,20,540],[-600,-40,320],[-700,50,-10],[-540,-20,-280],[-240,60,-380]] },
  ice_cathedral: { label:"Ice Cathedral",  desc:"Pianeta ghiacciato — cattedrali di cristallo.",      bg:0x020810, fog:0x081828, fogD:0.0011, amb:0x44ccee, sun:0x88eeff, nebula:false, planet:false, rockColor:0x6699bb, rocks:80, bgImg:"/assets/bg_ice_cathedral.jpeg", pts:[[0,0,0],[220,40,-190],[420,20,-160],[560,60,40],[490,30,290],[240,-30,430],[-20,50,390],[-260,70,450],[-480,20,270],[-580,-20,10],[-440,50,-230],[-200,-10,-310]] },
  solar_forge:   { label:"Solar Forge",    desc:"Vicino alla stella — plasma e eruzioni solari.",     bg:0x110400, fog:0x220800, fogD:0.0015, amb:0xff8800, sun:0xffcc44, nebula:false, planet:false, rockColor:0x994422, rocks:60, bgImg:"/assets/bg_solar_forge.png",  pts:[[0,0,0],[240,10,-210],[440,-20,-140],[570,30,70],[490,10,310],[240,-50,450],[-40,20,410],[-290,50,460],[-510,-10,260],[-600,20,0],[-460,-30,-240],[-210,10,-320]] },
  ghost_nebula:  { label:"Ghost Nebula",   desc:"Nebulosa spettrale — gas fantasma e stelle morenti.", bg:0x030a04, fog:0x0a1a0c, fogD:0.0009, amb:0x66cc88, sun:0xaaffcc, nebula:true,  planet:false, rockColor:0x446644, rocks:45, bgImg:"/assets/bg_ghost_nebula.jpeg", pts:[[0,0,0],[210,60,-170],[400,30,-140],[530,70,60],[460,40,300],[210,-20,440],[-30,60,400],[-270,80,460],[-490,30,280],[-570,-10,20],[-430,60,-210],[-190,20,-290]] },
};
...
function initGame(container, cfg, ui) {
  ...
  if(TR.masterplan)scene.traverse(o=>{if(o.isMesh){o.receiveShadow=true;o.castShadow=!o.material?.transparent&&(o.castShadow||o.name.startsWith('City block')||o.parent?.name==='Canair architectural frontage');}});
  ...
  function update(dt) {
    ...
    if (racing) {
      const bodies = racers.map(r => {
        ...
        return {y:r.mesh.position.y,previousY:r.previousY??r.mesh.position.y,x:r.mesh.position.x,z:r.mesh.position.z,vx:tangent.x*speed+r.kick.x,vz:tangent.z*speed+r.kick.z};
      });
  ...
  function pushHud() {
    ...
    ui.onHud({
      ...
      altitude:Math.round(player.mesh.position.y+7),
          ...
          <div className={"wr-hud" + (isTouch ? " wr-mobile" : "")}>
            ...
            {hud && isTouch && <div className="wr-mobile-stats">
              <div className="wr-mobile-line">
                ...
                <span>{TRACKS[trackKey].masterplan&&<small>QUOTA {hud.altitude} · </small>}{hud.speed} <small>VEL</small></span>
              </div>
